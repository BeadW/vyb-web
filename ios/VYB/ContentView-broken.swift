...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Text Styling Panel
            if let selectedTextLayer = layers.first(where: { $0.isSelected && $0.type == "text" }),
               let selectedIndex = layers.firstIndex(where: { $0.id == selectedTextLayer.id }) {
                VStack(alignment: .leading, spacing: 12) {
                    HStack {
                        ...
                        Spacer()
                        Button("Close") {
                            layers[selectedIndex].isSelected = false
                        }
                        .font(.system(size: 12))
                        .foregroundColor(.blue)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Font Size: \(Int(layers[selectedIndex].fontSize))")
                            .font(.system(size: 12, weight: .medium))
                        Slider(value: Binding(
                            get: { layers[selectedIndex].fontSize },
                            set: { layers[selectedIndex].fontSize = $0 }
                        ), in: 12...48, step: 1)
                    }
                            ...
                            HStack(spacing: 8) {
                                ForEach([Font.Weight.ultraLight, .light, .regular, .medium, .semibold, .bold, .heavy, .black], id: \\.self) { weight in
                                    Button(weightName(weight)) {
                                        layers[selectedIndex].fontWeight = weight
                                    }
                                    .font(.system(size: 11))
                                    .padding(.horizontal, 8)
                                    .padding(.vertical, 4)
                                    .background(layers[selectedIndex].fontWeight == weight ? Color.blue : Color.gray.opacity(0.2))
                                    .foregroundColor(layers[selectedIndex].fontWeight == weight ? .white : .primary)
                                    .cornerRadius(4)
                                }
                            }
                    ...
                    // Text Style Toggles
                    HStack(spacing: 16) {
                        Button(action: { layers[selectedIndex].isItalic.toggle() }) {
                            HStack(spacing: 4) {
                                Image(systemName: \"italic\")\n                                Text(\"Italic\")\n                            }\n                            .font(.system(size: 11))\n                            .padding(.horizontal, 8)\n                            .padding(.vertical, 4)\n                            .background(layers[selectedIndex].isItalic ? Color.blue : Color.gray.opacity(0.2))\n                            .foregroundColor(layers[selectedIndex].isItalic ? .white : .primary)\n                            .cornerRadius(4)\n                        }\n                        \n                        Button(action: { layers[selectedIndex].isUnderlined.toggle() }) {\n                            HStack(spacing: 4) {\n                                Image(systemName: \"underline\")\n                                Text(\"Underline\")\n                            }\n                            .font(.system(size: 11))\n                            .padding(.horizontal, 8)\n                            .padding(.vertical, 4)\n                            .background(layers[selectedIndex].isUnderlined ? Color.blue : Color.gray.opacity(0.2))\n                            .foregroundColor(layers[selectedIndex].isUnderlined ? .white : .primary)\n                            .cornerRadius(4)\n                        }\n                    }\n                    \n                    // Text Color\n                    VStack(alignment: .leading, spacing: 6) {\n                        Text(\"Text Color\")\n                            .font(.system(size: 12, weight: .medium))\n                        ScrollView(.horizontal, showsIndicators: false) {\n                            HStack(spacing: 8) {\n                                ForEach([Color.black, .white, .red, .blue, .green, .orange, .purple, .pink, .yellow], id: \\.self) { color in\n                                    Button(action: { layers[selectedIndex].textColor = color }) {\n                                        Circle()\n                                            .fill(color)\n                                            .frame(width: 24, height: 24)\n                                            .overlay(\n                                                Circle()\n                                                    .stroke(layers[selectedIndex].textColor == color ? Color.blue : Color.clear, lineWidth: 2)\n                                            )\n                                    }\n                                }\n                            }\n                            .padding(.horizontal, 4)\n                        }\n                    }\n                    \n                    // Text Effects\n                    VStack(alignment: .leading, spacing: 8) {\n                        Text(\"Effects\")\n                            .font(.system(size: 12, weight: .medium))\n                        \n                        HStack(spacing: 16) {\n                            Button(action: { layers[selectedIndex].hasShadow.toggle() }) {\n                                HStack(spacing: 4) {\n                                    Image(systemName: \"shadow\")\n                                    Text(\"Shadow\")\n                                }\n                                .font(.system(size: 11))\n                                .padding(.horizontal, 8)\n                                .padding(.vertical, 4)\n                                .background(layers[selectedIndex].hasShadow ? Color.blue : Color.gray.opacity(0.2))\n                                .foregroundColor(layers[selectedIndex].hasShadow ? .white : .primary)\n                                .cornerRadius(4)\n                            }\n                            \n                            Button(action: { layers[selectedIndex].hasStroke.toggle() }) {\n                                HStack(spacing: 4) {\n                                    Image(systemName: \"pencil.tip\")\n                                    Text(\"Stroke\")\n                                }\n                                .font(.system(size: 11))\n                                .padding(.horizontal, 8)\n                                .padding(.vertical, 4)\n                                .background(layers[selectedIndex].hasStroke ? Color.blue : Color.gray.opacity(0.2))\n                                .foregroundColor(layers[selectedIndex].hasStroke ? .white : .primary)\n                                .cornerRadius(4)\n                            }\n                        }\n                    }\n                    \n                    // Text Alignment\n                    VStack(alignment: .leading, spacing: 6) {\n                        Text(\"Alignment\")\n                            .font(.system(size: 12, weight: .medium))\n                        HStack(spacing: 8) {\n                            ForEach([TextAlignment.leading, .center, .trailing], id: \\.self) { alignment in\n                                Button(action: { layers[selectedIndex].textAlignment = alignment }) {\n                                    Image(systemName: alignmentIcon(alignment))\n                                        .font(.system(size: 16))\n                                        .padding(8)\n                                        .background(layers[selectedIndex].textAlignment == alignment ? Color.blue : Color.gray.opacity(0.2))\n                                        .foregroundColor(layers[selectedIndex].textAlignment == alignment ? .white : .primary)\n                                        .cornerRadius(4)\n                                }\n                            }\n                        }\n                    }\n                }\n                .padding(.horizontal, 16)\n                .padding(.vertical, 12)\n                .background(Color.orange.opacity(0.05))\n                .cornerRadius(8)\n                .padding(.horizontal, 16)\n                .padding(.top, 8)\n            }\n            \n            Divider()"
                        ...
                        Text(layer.content)
                            ...
                            .overlay(
                                Text(layer.content)
                                    ...
                                    .italic(layer.isItalic)
                                    .underline(layer.isUnderlined)
                                    .foregroundColor(layer.hasStroke ? layer.strokeColor : Color.clear)
                                    .multilineTextAlignment(layer.textAlignment) 
                                    .mask(
                                        Text(layer.content)
                                            .font(.system(size: layer.fontSize, weight: layer.fontWeight))
                                            ...
                                            .overlay(
                                                RoundedRectangle(cornerRadius: 0)
                                                    .stroke(Color.black, lineWidth: layer.hasStroke ? layer.strokeWidth : 0)
                                            )
                                    )
                            )
                    )
